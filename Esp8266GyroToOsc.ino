...
void loop() {
  ...
  Serial.print("Z: "); Serial.print(event.acceleration.z); Serial.print("  ");Serial.println("m/s^2 ");
    ...
    OscWiFi.send(host, send_port, "/accelX", event.acceleration.x);
    OscWiFi.update();  // should be called to receive + send osc
    OscWiFi.send(host, send_port, "/accelY", event.acceleration.y);
    OscWiFi.update();  // should be called to receive + send osc
    OscWiFi.send(host, send_port, "/accelZ", event.acceleration.z);
    OscWiFi.update();  // should be called to receive + send osc
