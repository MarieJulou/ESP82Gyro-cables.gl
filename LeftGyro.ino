...
void loop() {
    ...
    OscWiFi.send(host, send_port, "/StretchLeft", StretchValue);
    OscWiFi.update();  // should be called to receive + send osc
    
   OscWiFi.send(host, send_port, "/accelXLeft", event.acceleration.x);
    OscWiFi.update();  // should be called to receive + send osc
    OscWiFi.send(host, send_port, "/accelYLeft", event.acceleration.y);
    OscWiFi.update();  // should be called to receive + send osc
    OscWiFi.send(host, send_port, "/accelZLeft", event.acceleration.z);
    OscWiFi.update();  // should be called to receive + send osc
