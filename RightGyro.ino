...
const int analogStrechPin = A0;  // ESP8266 Analog Pin ADC0 = A0

int StretchValue = 0;  // value read from the pot
...
void loop() {
    ...
    StretchValue = analogRead(analogStrechPin);
    OscWiFi.send(host, send_port, "/StretchRight", StretchValue);
    OscWiFi.update();  // should be called to receive + send osc
    
   OscWiFi.send(host, send_port, "/accelXRight", event.acceleration.x);
    OscWiFi.update();  // should be called to receive + send osc
    OscWiFi.send(host, send_port, "/accelYRight", event.acceleration.y);
    OscWiFi.update();  // should be called to receive + send osc
    OscWiFi.send(host, send_port, "/accelZRight", event.acceleration.z);
    OscWiFi.update();  // should be called to receive + send osc
